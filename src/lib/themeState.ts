...
/** The live half of the theme: the level in effect in this browser, changing it, and a hook that follows it. */
...
/** Applies a level to the page. `animate` fades the colours over a fraction of a second, unless the reader prefers reduced motion. */
  ...
  return useSyncExternalStore(
    (onChange) => {
      ...
      const onStorage = (e: StorageEvent) => {
        if (e.key === STORAGE_KEY) applyLevel(readStoredLevel() ?? DEFAULT_LEVEL);
      };
