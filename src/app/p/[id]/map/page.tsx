...
import { layout, nodeTypes, RISK_COLOR, type NodeData } from "@/components/graph";
...
function MapInner() {
  ...
  const path = mode === "area" ? `/api/projects/${id}/graph?type=area` : mode === "module" ? `/api/projects/${id}/graph?type=module&limit=70${area ? `&area=${encodeURIComponent(area)}` : ""}` : symbol ? `/api/projects/${id}/graph?type=symbol&symbol=${encodeURIComponent(symbol)}&depth=${depth}` : null;
  ...
  const laid = useMemo(() => (data ? layout(data.graph, selected?.id ?? null, highlight) : { nodes: [], edges: [] }), [data, selected, highlight]);
        ...
        <div className="relative h-[65vh] min-w-0 flex-none lg:h-auto lg:flex-1" style={{ background: "var(--bg)" }}>
          {mode === "symbol" && !symbol ? <Empty title="Choose a symbol to explore">Open a file in the Files view and pick a symbol, or select a file here in Files mode and choose one of its symbols.</Empty>
            : loading && !data ? <Loading label="Building graph" /> : error ? <ErrorBox error={error} onRetry={reload} /> : !data || data.graph.nodes.length === 0 ? <Empty title="Nothing to draw">No relationships were found at this level.</Empty> : (
              <ReactFlow nodes={laid.nodes} edges={laid.edges} nodeTypes={nodeTypes} fitView fitViewOptions={{ padding: 0.12, maxZoom: 1.25 }} minZoom={0.15} maxZoom={1.8} nodesConnectable={false} proOptions={{ hideAttribution: true }}
                ...
                {laid.nodes.length > 25 && <MiniMap pannable zoomable nodeColor={(n) => { const r = (n.data as NodeData).risk; return r === "none" ? "var(--muted)" : RISK_COLOR[r]; }} maskColor="rgba(0,0,0,0.08)" style={{ background: "var(--panel)" }} />}
              </ReactFlow>
            )}
          <details open className="absolute bottom-3 left-3 z-10 max-w-[520px]"><summary className="btn mb-1 w-fit cursor-pointer text-xs">Legend</summary><Legend compact /></details>
        </div>
